...
Renter.prototype.getContact = function () {
  return this.network ? this.network.contact : storj.Contact({
    address: this._opts.networkOpts.rpcAddress,
    port: this._opts.networkOpts.rpcPort,
    nodeID: storj.KeyPair(this._opts.networkPrivateKey).getNodeID()
  });
};

Renter.prototype.addSeedList = function (seedContact) {
  if (seedContact.nodeID != this.getContact().nodeID) {
    this._opts.networkOpts.seedList.push(storj.utils.getContactURL(seedContact))
  }
};
